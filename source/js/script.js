...
var cols = document.getElementsByClassName('reviews__list-item');
var block = document.querySelector('#block');
var prev = document.querySelector('#prev');
var next = document.querySelector('#next');
var minMaxWidth = window.matchMedia('(max-width: 550px)');
var maxWidth = window.matchMedia('(min-width: 551px) and (max-width: 767px)');
...
//Функция перемотки слайдов влево
var moveToRight = function() {
  var number_slide = block.dataset.slide;

  if (number_slide == 1) {
    var position = (slides - 2) * size;
    block.style.transform = "translate(-"+position+"%,0)"
    block.dataset.slide = parseInt(slides) - 1;
  }
  if (number_slide != 1) {
    var position = (number_slide - 2) * size;
    block.style.transform = "translate(-"+position+"%,0)";
    block.dataset.slide = parseInt(number_slide) - 1;
  }
  prev.style.display = 'block';
  next.style.display = 'none';
};

//Функция перемотки слайдов вправо
var moveToLeft = function() {
  var number_slide = block.dataset.slide;

  var position = number_slide * size;
  if (number_slide < slides - 1) {
    block.style.transform = "translate(-"+position+"%,0)";
    block.dataset.slide = parseInt(number_slide) + 1;
  } else {
    block.style.transform = "translate(0%,0)"
    block.dataset.slide = 1;
  }
  prev.style.display = 'none';
  next.style.display = 'block';
};

var moveToRightMobile = function() {
 var number_slide = block.dataset.slide;
...
next.addEventListener('click', function(evt) {
  evt.preventDefault;
  mobileWidth.matches ? moveToRightMobile() : moveToRight();
});

prev.addEventListener('click', function(evt) {
  evt.preventDefault;
  mobileWidth.matches ? moveToLeftMobile() : moveToLeft();
});
...
var linkInstruction = document.querySelector('.instruction__button');
